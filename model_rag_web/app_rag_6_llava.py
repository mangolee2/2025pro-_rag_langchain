...
OLLAMA_MODEL = "llava"        # 로컬에서 돌아가는 모델: 멀티모달 씀 
...
def rag_answer_questions(questions, retriever, chunks, embedder):
    ...
    for q in questions:
        ...
        prompt = (
            ...
            f"질문: {q}\n"
            f"요구사항:\n"
            # f"extract generic names in papers and explain them."  
            f"don't read the reference part of papers."
            # f"extract only generic names in papers, not brand names."  
            f"Extract only the key anticancer drugs (using their generic names) mentioned in the paper."
            f"List only the essential generic names"
            f"응답:"
        )
