...
def rag_answer_questions(questions, retriever, chunks, embedder):
    ...
    for q in questions:
        ...
        prompt = (
            ...
            f"3) 가능하면 근거 문장(문서에서 발췌한 문장)을 짧게 함께 덧붙일 것\n"
            f"4) 종합테이블 형태로 3가지 답 정리할 것\n"
            f"응답:"
        )
...
if uploaded_file and run_button:
    ...
    if not text:
        ...
        # 자동질문 리스트
        questions = [
            "논문에서 언급된 신약명(후보)을 알려줘.",
            "이 신약의 효능(responsive)에 대해 설명해줘 (어떤 적응증/반응을 보였는지).",
            "이 신약의 독성(toxic) 관련 언급이 있는지, 있다면 어떤 독성인지 말해줘."
        ]
