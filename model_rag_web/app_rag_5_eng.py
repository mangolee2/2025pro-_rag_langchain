...
# 시스템 메시지 정의
system_message = SystemMessage(content="you are a reasearcher for new drug development related to mouse. You have to explain generic names of drugs")
...
def rag_answer_questions(questions, retriever, chunks, embedder):
    ...
    for q in questions:
        ...
        # 프롬프트 템플릿: 명확한 포맷 요청
        prompt = (
            f"다음 문서 발췌문을 참고하여 아래 질문에 답하세요.\n\n"
            f"==== 문서 발췌 시작 ====\n{context}\n==== 문서 발췌 끝 ====\n\n"
            f"질문: {q}\n"
            f"요구사항:\n"
            # f"1) 간결하게 핵심만 요약\n"
            # f"2) 결과를 '항목명: 값' 형태로 출력\n"
            # f"가능하면 근거 문장(문서에서 발췌한 문장)을 짧게 함께 덧붙일 것\n"
            f"extract generic names of drugs and explain them"
            # f"유효성(responsive)이란, 암의 무게/부피의 감소한 수치를 보여주거나 감소율(%) 보여줄 것\n"
            # f"독성(toxic)이란, 전체 쥐 중 몇 마리가 사망했는지 사망률(%) 혹은 o,x(binary) 형태로 보여줄 것\n"
            # f"위 3가지를 정리한 후에, 종합 정리 테이블도 출력할 것 \n"
            f"응답:"
        )
...
st.markdown("업로드 된 논문(PDF)에서 **신약명** 항목을 자동으로 추출합니다. Ollama(gpt-oss)와 로컬 임베딩+FAISS를 사용한 RAG 방식입니다.")
...
if uploaded_file and run_button:
    ...
    if not text:
        ...
        questions = [
            # "논문제목 알려주고 시작해"
            "tell me generic names of drugs mentioned in the paper",
            # "각 신약의 유효성(responsive)에 대해 설명해줘.",
            # "각 신약의 독성(toxic) 관련 정보를 말해줘.",
            # "종합 테이블 형태로 정리해줘."
        ]
